...
namespace bjou {
    ...
    constexpr const TokenParserFnType tokenParsers[] = {
        parser_generic_token,
        parser_non_comment,
        parser_identifier,
        parser_identifier_allow_primative_types,
        parser_any_char,
        parser_semicolon,
        parser_under_score,
        parser_comma,
        ...
        parser_question,
        parser_dbl_question,
        parser_template_begin,
        parser_kwd_const,
        parser_kwd_type,
        parser_kwd_abstract,
        parser_kwd_extends,
        parser_kwd_interface,
        parser_kwd_implements,
        parser_kwd_from,
        parser_kwd_enum,
        parser_kwd_print,
        parser_kwd_raw,
        ...
        parser_kwd_break,
        parser_kwd_continue,
        parser_l_curly_brace,
        parser_r_curly_brace,
        parser_l_sqr_bracket,
        parser_r_sqr_bracket,
        parser_l_paren,
        parser_r_paren,
        parser_integer,
        parser_floating_pt,
        ...
        parser_sl_comment_beg,
        parser_ml_comment_beg,
        parser_ml_comment_end,
        parser_end_of_line
    };
    
#define GOOD_IDX(buff, p) ((p) < (buff).viewSize())
#define IS_C(buff, p, c)                                \
    ((p) < (buff).viewSize() &&                         \
    (buff)[(p)] == (c))
...
#define IS_aZ09(buff, p)                                \
    ...
    (((buff)[(p)]) >= 'A' && ((buff)[(p)]) <= 'Z')) ||  \
     (((buff)[(p)]) >= '0' && ((buff)[(p)]) <= '9')))
#define IS_SPACE(buff, p)                               \
    ((p) < (buff).viewSize() &&                         \
    (isspace((buff)[(p)])))
    
    const char * kwds[] = {
        "type", "abstract", "interface", "implements", "extends", "from", "enum", "print", "raw",
        "immut", "delete", "namespace", "import", "alias",
        "operator", "coerce", "this",
        
        "return", "if", "else", "while", "do", "for", "match",
        "with", "break", "continue",
        
        "true", "false", "nothing",
        
        "as", "sizeof", "not", "new", "proc", "extern", "some",
        "and", "or"
    };
    ...
    MaybeString parser_generic_token(StringViewableBuffer& buff) {
        size_t p = 0;
        if (GOOD_IDX(buff, p) && !IS_aZ09(buff, p) && !IS_C(buff, p, '_') && !IS_SPACE(buff, p))
            return MaybeString(buff.substr(0, 1));
        else while (GOOD_IDX(buff, p) && (IS_aZ09(buff, p) || IS_C(buff, p, '_')))//!IS_SPACE(buff, p))
            p += 1;
    ...
    MaybeString parser_identifier_allow_primative_types(StringViewableBuffer& buff) {
        bool has_non_underscore = false;
        size_t p = 0;
        if (!(IS_aZ(buff, p) || IS_C(buff, p, '_')))
            return MaybeString();
    ...
    MaybeString parser_semicolon(StringViewableBuffer& buff) { return parse_punc(buff, ";"); }
    MaybeString parser_under_score(StringViewableBuffer& buff) { return parse_punc(buff, "_"); }
    MaybeString parser_comma(StringViewableBuffer& buff) { return parse_punc(buff, ","); }
    MaybeString parser_ellipsis(StringViewableBuffer& buff) { return parse_punc(buff, "..."); }
    MaybeString parser_colon(StringViewableBuffer& buff) { return parse_punc(buff, ":"); }
    MaybeString parser_dbl_colon(StringViewableBuffer& buff) { return parse_punc(buff, "::"); }
    MaybeString parser_back_slash(StringViewableBuffer& buff) { return parse_punc(buff, "\\"); }
    MaybeString parser_vert(StringViewableBuffer& buff) { return parse_punc(buff, "|"); }
    MaybeString parser_dot_colon(StringViewableBuffer& buff) { return parse_punc(buff, ".:"); }
    MaybeString parser_hash(StringViewableBuffer& buff) { return parse_punc(buff, "#"); }
    MaybeString parser_thick_arrow(StringViewableBuffer& buff) { return parse_punc(buff, "=>"); }
    MaybeString parser_dollar(StringViewableBuffer& buff) { return parse_punc(buff, "$"); }
    MaybeString parser_question(StringViewableBuffer& buff) { return parse_punc(buff, "?"); }
    MaybeString parser_dbl_question(StringViewableBuffer& buff) { return parse_punc(buff, "??"); }
    ...
    MaybeString parser_kwd_const(StringViewableBuffer& buff) { return parse_kwd(buff, "const"); }
    MaybeString parser_kwd_type(StringViewableBuffer& buff) { return parse_kwd(buff, "type"); }
    MaybeString parser_kwd_abstract(StringViewableBuffer& buff) { return parse_kwd(buff, "abstract"); }
    MaybeString parser_kwd_extends(StringViewableBuffer& buff) { return parse_kwd(buff, "extends"); }
    MaybeString parser_kwd_interface(StringViewableBuffer& buff) { return parse_kwd(buff, "interface"); }
    MaybeString parser_kwd_implements(StringViewableBuffer& buff) { return  parse_kwd(buff, "implements"); }
    MaybeString parser_kwd_from(StringViewableBuffer& buff) { return  parse_kwd(buff, "from"); }
    MaybeString parser_kwd_enum(StringViewableBuffer& buff) { return  parse_kwd(buff, "enum"); }
    MaybeString parser_kwd_print(StringViewableBuffer& buff) { return  parse_kwd(buff, "print"); }
    MaybeString parser_kwd_raw(StringViewableBuffer& buff) { return  parse_kwd(buff, "raw"); }
    MaybeString parser_kwd_immut(StringViewableBuffer& buff) { return  parse_kwd(buff, "immut"); }
    MaybeString parser_kwd_coerce(StringViewableBuffer& buff) { return  parse_kwd(buff, "coerce"); }
    MaybeString parser_kwd_this(StringViewableBuffer& buff) { return  parse_kwd(buff, "this"); }
    MaybeString parser_kwd_delete(StringViewableBuffer& buff) { return  parse_kwd(buff, "delete"); }
    MaybeString parser_kwd_namespace(StringViewableBuffer& buff) { return  parse_kwd(buff, "namespace"); }
    MaybeString parser_kwd_import(StringViewableBuffer& buff) { return  parse_kwd(buff, "import"); }
    MaybeString parser_kwd_module(StringViewableBuffer& buff) { return  parse_kwd(buff, "module"); }
    MaybeString parser_kwd_alias(StringViewableBuffer& buff) { return  parse_kwd(buff, "alias"); }
    MaybeString parser_kwd_operator(StringViewableBuffer& buff) { return  parse_kwd(buff, "operator"); }
    MaybeString parser_kwd_return(StringViewableBuffer& buff) { return  parse_kwd(buff, "return"); }
    MaybeString parser_kwd_if(StringViewableBuffer& buff) { return  parse_kwd(buff, "if"); }
    MaybeString parser_kwd_else(StringViewableBuffer& buff) { return  parse_kwd(buff, "else"); }
    MaybeString parser_kwd_while(StringViewableBuffer& buff) { return  parse_kwd(buff, "while"); }
    MaybeString parser_kwd_do(StringViewableBuffer& buff) { return  parse_kwd(buff, "do"); }
    MaybeString parser_kwd_for(StringViewableBuffer& buff) { return  parse_kwd(buff, "for"); }
    MaybeString parser_kwd_match(StringViewableBuffer& buff) { return  parse_kwd(buff, "match"); }
    MaybeString parser_kwd_with(StringViewableBuffer& buff) { return  parse_kwd(buff, "with"); }
    MaybeString parser_kwd_break(StringViewableBuffer& buff) { return  parse_kwd(buff, "break"); }
    MaybeString parser_kwd_continue(StringViewableBuffer& buff) { return  parse_kwd(buff, "continue"); }
    MaybeString parser_l_curly_brace(StringViewableBuffer& buff) { return parse_punc(buff, "{"); }
    MaybeString parser_r_curly_brace(StringViewableBuffer& buff) { return parse_punc(buff, "}"); }
    MaybeString parser_l_sqr_bracket(StringViewableBuffer& buff) { return parse_punc(buff, "["); }
    MaybeString parser_r_sqr_bracket(StringViewableBuffer& buff) { return parse_punc(buff, "]"); }
    MaybeString parser_l_paren(StringViewableBuffer& buff) { return parse_punc(buff, "("); }
    MaybeString parser_r_paren(StringViewableBuffer& buff) { return parse_punc(buff, ")"); }
    ...
    MaybeString parser_string_literal(StringViewableBuffer& buff) {
        ...
        while (true) {
            ...
            if (p < buff.viewSize())
                p += 1;
            else return MaybeString();
        }
        return MaybeString(buff.substr(0, p));
    }
    MaybeString parser_kwd_true(StringViewableBuffer& buff) { return  parse_kwd(buff, "true"); }
    MaybeString parser_kwd_false(StringViewableBuffer& buff) { return  parse_kwd(buff, "false"); }
    MaybeString parser_kwd_nothing(StringViewableBuffer& buff) { return  parse_kwd(buff, "nothing"); }
    MaybeString parser_kwd_as(StringViewableBuffer& buff) { return  parse_kwd(buff, "as"); }
    MaybeString parser_dot(StringViewableBuffer& buff) { return parse_punc(buff, "."); }
    MaybeString parser_arrow(StringViewableBuffer& buff) { return parse_punc(buff, "->"); }
    MaybeString parser_exclam(StringViewableBuffer& buff) { return parse_punc(buff, "!"); }
    MaybeString parser_kwd_sizeof(StringViewableBuffer& buff) { return  parse_kwd(buff, "sizeof"); }
    MaybeString parser_amp(StringViewableBuffer& buff) { return parse_punc(buff, "&"); }
    MaybeString parser_at(StringViewableBuffer& buff) { return parse_punc(buff, "@"); }
    MaybeString parser_kwd_not(StringViewableBuffer& buff) { return  parse_kwd(buff, "not"); }
    MaybeString parser_kwd_new(StringViewableBuffer& buff) { return  parse_kwd(buff, "new"); }
    MaybeString parser_kwd_proc(StringViewableBuffer& buff) { return  parse_kwd(buff, "proc"); }
    MaybeString parser_kwd_extern(StringViewableBuffer& buff) { return  parse_kwd(buff, "extern"); }
    MaybeString parser_kwd_some(StringViewableBuffer& buff) { return  parse_kwd(buff, "some"); }
    MaybeString parser_mult(StringViewableBuffer& buff) { return parse_punc(buff, "*"); }
    MaybeString parser_div(StringViewableBuffer& buff) { return parse_punc(buff, "/"); }
    MaybeString parser_mod(StringViewableBuffer& buff) { return parse_punc(buff, "%"); }
    MaybeString parser_plus(StringViewableBuffer& buff) { return parse_punc(buff, "+"); }
    MaybeString parser_minus(StringViewableBuffer& buff) { return parse_punc(buff, "-"); }
    MaybeString parser_lss(StringViewableBuffer& buff) { return parse_punc(buff, "<"); }
    MaybeString parser_leq(StringViewableBuffer& buff) { return parse_punc(buff, "<="); }
    MaybeString parser_gtr(StringViewableBuffer& buff) { return parse_punc(buff, ">"); }
    MaybeString parser_geq(StringViewableBuffer& buff) { return parse_punc(buff, ">="); }
    MaybeString parser_equ(StringViewableBuffer& buff) { return parse_punc(buff, "=="); }
    MaybeString parser_neq(StringViewableBuffer& buff) { return parse_punc(buff, "!="); }
    MaybeString parser_and(StringViewableBuffer& buff) { return parse_punc(buff, "&&"); }
    MaybeString parser_kwd_and(StringViewableBuffer& buff) { return  parse_kwd(buff, "and"); }
    MaybeString parser_or(StringViewableBuffer& buff) { return parse_punc(buff, "||"); }
    MaybeString parser_kwd_or(StringViewableBuffer& buff) { return  parse_kwd(buff, "or"); }
    MaybeString parser_assign(StringViewableBuffer& buff) { return parse_punc(buff, "="); }
    MaybeString parser_plus_eq(StringViewableBuffer& buff) { return parse_punc(buff, "+="); }
    MaybeString parser_min_eq(StringViewableBuffer& buff) { return parse_punc(buff, "-="); }
    MaybeString parser_mult_eq(StringViewableBuffer& buff) { return parse_punc(buff, "*="); }
    MaybeString parser_div_eq(StringViewableBuffer& buff) { return parse_punc(buff, "/="); }
    MaybeString parser_mod_eq(StringViewableBuffer& buff) { return parse_punc(buff, "%="); }
    ...
    MaybeString parser_sl_comment_beg(StringViewableBuffer& buff) { return parse_punc(buff, "#"); }
    MaybeString parser_ml_comment_beg(StringViewableBuffer& buff) { return MaybeString(); }
    MaybeString parser_ml_comment_end(StringViewableBuffer& buff) { return MaybeString(); }
    ...
    Parser::Parser(std::ifstream& file, const std::string& fname, bool start) : buff(file) {
    ...
    void Parser::parseCommon() {
        ...
        while (buff.viewSize() > 0) {
            ...
            if (node->nodeKind == ASTNode::MODULE_DECL) {
                moduleCheck(compilation->frontEnd.AST, module_declared, (ModuleDeclaration*)node);
    ...
    void Parser::clean() {
        ...
        while (IS_SPACE(buff, 0)) {
            ...
            } else currentContext.end.character += 1;
            buff.advance(1);
            justCleanedContext.finish(&currentContext);
            altered = true;
        }
        if (!altered) justCleanedContext = save_justCleanedContext;
    ...
    bjou::Maybe<std::string> Parser::optional(TokenKind tok, bool skipEat, bool skipClean) {
        ...
        if (m_match.assignTo(match)) {
            if (!skipEat)
                ...
                clean();
            if (tok == END_OF_LINE) {
                // newlines won't be taken by clean() if they are specifically looked for
                // but the currentContext still needs to be updated
                currentContext.end.line += 1;
                currentContext.end.character = 1;
            }
        }
    ...
    // There is also a Parser::expect defined in Parser.hpp (it's a template) that takes
    // continuations for reporting errors
    // just thought you'd like to know
    std::string Parser::expect(TokenKind tok, std::string err, bool skipEat, bool skipClean) {
    ...
    MaybeASTNode Parser::parseDeclarator() {
        ...
        if (optional(LSS)) { // Procedure
            ...
            while (!optional(R_PAREN)) {
                if (optional(ELLIPSIS)) {
                    ...
                    if (vararg)
                        errornext(*this, "Vararg ('...') denotation must be the last argument.");
                    ...
                    if (!m_paramDeclarator.assignTo(paramDeclarator))
                        errornext(*this, "Expected type declarator in argument list for procedure type declarator.");
            ...
            if (!optional(COLON)) {
                ...
                if (!m_retDeclarator.assignTo(retDeclarator))
                    errornext(*this, "Expected return type declarator for prodecure type declarator.");
            ...
            while (!optional(R_PAREN)) {
                ...
                if (!m_subDeclarator.assignTo(subDeclarator))
                    errornext(*this, "Expected type declarator as sub type in tuple declarator.");
            ...
            if (optional(TEMPLATE_BEGIN, true)) {
                ...
                else errornext(*this, "Expected template instantiation after '!'.");
            }
            ...
            context.finish(&currentContext, &justCleanedContext);
            result->setContext(context);
        } else return MaybeASTNode();
        
        // Array, Pointer, Maybe
        //                array - [               pointer - *                maybe - ?
        while (optional(L_SQR_BRACKET, true) || optional(MULT, true) || optional(QUESTION, true)) {
            if (optional(L_SQR_BRACKET)) {
                if (optional(ELLIPSIS)) {
                    ...
                    m_expr.assignTo(expr); // this doesn't have to work since the expr is optional
        ...
        if (tuple_sub_err)
            errorl(result->getContext(), "Tuple types must have at least 2 sub types.");
        ...
        if (optional(KWD_FROM)) {
            while (true) {
                ...
                if (!m_decl.assignTo(decl))
                    errornext(*this, "Expected type declarator in template type descriptor.", true, "as template bound after 'from'");
    ...
    MaybeASTNode Parser::parseTemplateDefineVariadicTypeArgs() {
        ...
        if (optional(ELLIPSIS, true)) {
            TemplateDefineVariadicTypeArgs * result = new TemplateDefineVariadicTypeArgs();
            result->getContext().start(&currentContext);
    ...
    MaybeASTNode Parser::parseTemplateDef() {
        if (optional(TEMPLATE_BEGIN, true)) {
            ...
            if (result->getElements().size() == 0)
                errorl(result->getContext(), "Empty template definition list not allowed.");
    ...
    MaybeASTNode Parser::parseTemplateInst() {
        ...
        while (!optional(R_PAREN, true)) {
            ...
            if (optional(L_SQR_BRACKET)) {
                ...
                if (!m_element.assignTo(element))
                    errornext(*this, "Invalid expression argument in template instantiation.");
                expect(R_SQR_BRACKET, "']'");
            } else {
                m_element = parseDeclarator();
                if (!m_element.assignTo(element))
                    errornext(*this, "Invalid argument in template instantiation.", true, "Note: Expression arguments should be surrounded by brackets '([expr])'");
            }
    ...
    MaybeASTNode Parser::parseQualifiedIdentifier(bool allow_primative_typenames) {
        MaybeString m_identifier = optional(allow_primative_typenames ? IDENTIFIER_ALLOW_PRIMATIVE_TYPES : IDENTIFIER, true);
        std::string identifier;
        ...
        if (m_identifier.assignTo(identifier)) {
            ...
            expect(allow_primative_typenames ? IDENTIFIER_ALLOW_PRIMATIVE_TYPES : IDENTIFIER, "");
            
            while (optional(DBL_COLON)) {
                result->addNamespace(identifier);
                identifier = expect(IDENTIFIER, "identifier", false, false, "for qualified identifier");
            }
    ...
    static BinaryExpression * newBinaryExpressionFromOp(std::string& op) {
        if (op == "()")         return new CallExpression;
        else if (op == "[]")    return new SubscriptExpression;
        else if (op == ".")     return new AccessExpression;
        else if (op == "->")    return new InjectExpression;
        else if (op == "*")     return new MultExpression;
        else if (op == "/")     return new DivExpression;
        else if (op == "%")     return new ModExpression;
        else if (op == "+")     return new AddExpression;
        else if (op == "-")     return new SubExpression;
        else if (op == "<")     return new LssExpression;
        else if (op == "<=")    return new LeqExpression;
        else if (op == ">")     return new GtrExpression;
        else if (op == ">=")    return new GeqExpression;
        else if (op == "==")    return new EquExpression;
        else if (op == "!=")    return new NeqExpression;
        else if (op == "&&" ||
                 op == "and")   return new LogAndExpression;
        else if (op == "||" ||
                 op == "or")    return new LogOrExpression;
        else if (op == "??")    return new MaybeAssignExpression;
        else if (op == "=")     return new AssignmentExpression;
        else if (op == "*=")    return new MultAssignExpression;
        else if (op == "/=")    return new DivAssignExpression;
        else if (op == "%=")    return new ModAssignExpression;
        else if (op == "+=")    return new AddAssignExpression;
        else if (op == "-=")    return new SubAssignExpression;
        else internalError("bad operator '" + op + "'");
        return nullptr;
    }
    
    static UnaryPreExpression * newUnaryPreExpressionFromOp(std::string& op) {
        if (op == "!" || op == "not")    return new NotExpression;
        else if (op == "new")    return new NewExpression;
        else if (op == "delete") return new DeleteExpression;
        else if (op == "sizeof") return new SizeofExpression;
        else if (op == "proc") return new ProcLiteral;
        else if (op == "extern") return new ExternLiteral;
        else if (op == "some") return new SomeLiteral;
        else if (op == "@") return new DerefExpression;
        else if (op == "&") return new AddressExpression;
        else internalError("bad operator '" + op + "'");
        return nullptr;
    }
    ...
    MaybeASTNode Parser::parseExpression_r(ASTNode * left, int minPrecedence) {
        ...
        while (true) {
            ...
            if (op == "[]") {
                ...
                if (!m_right) //.assignTo(right)) // this won't work because we have to force an ArgList where an Expression should go
                    return MaybeASTNode();
                // force it
                ...
                expect(R_PAREN, "')'", false, false, optional(COLON) ? "precede procedure declarations with the keyword 'proc'" : "to end procedure call");
            } else {
                eat(op);
                if (binary(op.c_str())) {
                    m_right = parseOperand();
            ...
            while (true) {
                ...
                if (! // good luck reading this
                    ...
                    (rightAssoc(lookahead.c_str()) && precedence(lookahead.c_str()) == precedence(op.c_str())))))
                    break;
                /*
            ...
            if (m_postfixOp.assignTo(postfixOp)) {
                result = (Expression*)applyPostfixOp(postfixOp, result, (Expression*)left, right, m_op, minPrecedence);
                BJOU_DEBUG_ASSERT(result);
            }
    ...
    MaybeString Parser::parseBinaryOperator(bool skipEat) {
        const TokenKind opOrder[] = {
            DOT, ARROW, L_SQR_BRACKET, L_PAREN, PLUS_EQ, MIN_EQ, MULT_EQ,
            DIV_EQ, MOD_EQ, MULT, DIV, MOD, PLUS, MINUS, LEQ,
            LSS, GEQ, GTR, EQU, NEQ, ASSIGN, AND, KWD_AND,
            OR, KWD_OR, DBL_QUESTION
        };
    ...
    MaybeString Parser::parseUnaryPrefixOperator(bool skipEat) {
        const TokenKind opOrder[] = {
            EXCLAM, KWD_NOT, KWD_NEW, KWD_DELETE, KWD_SIZEOF, KWD_PROC, KWD_EXTERN,
            KWD_SOME, AT, AMP
        };
    ...
    ASTNode * Parser::applyPostfixOp(std::string& postfixOp, Expression * expr, Expression * left, Expression * right, MaybeString& m_binOp, int minPrecedence) {
        /*
             Here, we try to skip applying a postfix op if the current binop is of a higher precedence
             (access, call, etc..). Otherwise, if there is no binop or if we've reached the end of the
             expression and the current binop is higher precedence (see Operator.hpp for this number),
             we apply the postfix to the whole thing. If it is of lower precedence, we only apply it to
             the immediate sub expression.
         */
        ...
        eat(postfixOp);
        UnaryPostExpression * postfix = postfixOp == "as" ? new AsExpression() : nullptr;// new UnaryPostExpression(); // @expr
        ...
        if (!m_binOp || (minPrecedence == 0 && precedence(binOp.c_str()) > 7)) {
            ...
            if (postfixOp == "as") {
                ...
                ASTNode * decl = nullptr;
                if (!m_decl.assignTo(decl))
                    errornext(*this, "Expected type declarator for 'as' type cast.");
    ...
    MaybeASTNode Parser::parseTerminatingExpression() {
        ...
        for (TokenKind kind : { KWD_TRUE, KWD_FALSE, KWD_NOTHING,
                                FLOATING_PT, INTEGER, CHAR_LITERAL,
                                STRING_LITERAL }) {
        ...
        } else if ((m_node = parseInitializerList()) || (m_node = parseParentheticalExpressionOrTuple())) {
            m_node.assignTo(node);
            BJOU_DEBUG_ASSERT(node);
        } else return MaybeASTNode();
    ...
    MaybeASTNode Parser::parseParentheticalExpressionOrTuple() {
        ...
        if (optional(L_PAREN)) {
            ...
            if (!m_expression.assignTo(expression)) {
                ...
                else errornext(*this, "Invalid expression.");
            }
            if (optional(COMMA)) { // tuple
                ...
                expression->getContext().finish(&currentContext, &justCleanedContext);
            ...
            if (optional(R_CURLY_BRACE)) {
                ...
                if (m_objDeclarator.assignTo(objDeclarator)) {
                    ...
                    while (!optional(R_CURLY_BRACE)) {
                        expect(DOT, "'.<member name>' for type member initialization");
                        std::string name = expect(IDENTIFIER, "type member name");
                        expect(ASSIGN, "'=' for type member initialization");
                        MaybeASTNode m_expression = parseExpression();
                        ASTNode * expression = nullptr;
                        if (!m_expression.assignTo(expression))
                            errornext(*this, "Invalid expression in initializer list.");
                    ...
                    while (!optional(R_CURLY_BRACE)) {
                        ...
                        ASTNode * expression = nullptr;
                        if (!m_expression.assignTo(expression))
                            errornext(*this, "Invalid expression in array initializer list.");
                        result->addExpression(expression);
                        if (!optional(COMMA)) {
                            expect(R_CURLY_BRACE, "'}'");
                            break;
    ...
    MaybeASTNode Parser::parseOperand() {
        ...
        if (m_op.assignTo(op)) {
            if (op == "new") {
                ...
                ASTNode * decl = nullptr;
                if (!m_decl.assignTo(decl))
                    errornext(*this, "Expected type declarator.", true, "after '" + op + "'");
                ...
                ASTNode * expression = nullptr;
                if (!m_expression.assignTo(expression))
                    errornext(*this, "Expected expression.", true, "after '" + op + "'");
                ...
                if (proc->getFlag(ASTNode::IS_TEMPLATE)) {
                    ...
                    errorl(templateProc->getTemplateDef()->getContext(), "Template procedures are not allowed as part of an expression.");
                }
                ...
                if (!m_externSig.assignTo(externSig))
                    errornext(*this, "Expected procedure signature after 'extern'.");
    ...
    MaybeASTNode Parser::parseTopLevelNode() {
        ...
        (m_node = parseNamespace())             ||
        (m_node = parseProc())                  ||
        (m_node = parseExternSig())             ||
        (m_node = parseType())                  ||
        (m_node = parseInterfaceDef())          ||
        (m_node = parseEnum())                  ||
        (m_node = parseConstant())              ||
        (m_node = parseVariableDeclaration())   ||
        (m_node = parseAlias())                 ||
        (m_node = parseStatement())             ||
        (m_node = parseImport());
    ...
    MaybeASTNode Parser::parseNamespace() {
        if (optional(KWD_NAMESPACE, true)) {
            ...
            expect(R_CURLY_BRACE, "'}'", false, false, "to close namespace '" + result->getName() + "'");
    ...
    MaybeASTNode Parser::parseType() {
        if (optional(KWD_ABSTRACT, true) || optional(KWD_TYPE, true)) {
			...
			m_templateDef.assignTo(templateDef); // we don't really care what this returns
			...
			if (m_aliasDeclarator.assignTo(aliasDeclarator)) {
				if (abstract)
					errorl(nameContext, "Only struct types can be abstract.", true, "'" + name + "' is an alias type.");
                ...
                if (optional(KWD_EXTENDS)) {
                    ...
                    if (!m_extends.assignTo(extends))
                        errornext(*this, "Expected type declarator after 'extends'.");
                    result->setExtends(extends);
                }
                
                if (result->getExtends())
                    expect(L_CURLY_BRACE, "'{'");
                else expect(L_CURLY_BRACE, "'{' for struct type or a valid declarator for an alias type");
                    ...
                    if (m_member.assignTo(member)) {
                        ...
                        if (m_member.assignTo(member)) {
                            ...
                            if (m_member.assignTo(member)) {
                                ...
                                else result->addMemberProc(member);
                            } else {
    ...
    MaybeASTNode Parser::parseInterfaceImpl() {
        if (optional(KWD_IMPLEMENTS, true)) {
            ...
            if (!m_qualIdentifier.assignTo(qualIdentifier))
                errornext(*this, "Expected identifier referring to an interface.", true, "after 'implements'");
            ...
            if (optional(L_CURLY_BRACE)) {
                ...
                expect(R_CURLY_BRACE, "'}'");
            } else result->setFlag(InterfaceImplementation::PUNT_TO_EXTENSION, true);
    ...
    MaybeASTNode Parser::parseAlias() {
        if (optional(KWD_ALIAS, true)) {
            ...
            if (!m_declarator.assignTo(declarator))
                errornext(*this, "Expected valid type declarator for type alias '" + result->getName() + "'.");
    ...
    MaybeASTNode Parser::parseConstant() {
        // this uses a special case parsing function so that we don't have to do any backtracking
        if (optional(KWD_CONST, true)) {
            ...
            if (optional(ASSIGN)) {
                ...
                if (!m_initialization.assignTo(initialization))
                    errornext(*this, "Invalid expression in initialization of constant '" + result->getName() + "'.");
                result->setInitialization(initialization);
            } else {
                MaybeASTNode m_declarator = parseDeclarator();
                ASTNode * declarator = nullptr;
                if (!m_declarator.assignTo(declarator))
                    errornext(*this, "Expected type declarator in declaration of constant '" + result->getName() + "'.");
        ...
        /*// this uses a special case parsing function so that we don't have to do any backtracking
        if (optional(CONSTANT_DECL_BEG, true)) {
            Constant * result = new Constant();
            result->getContext().start(&currentContext);
    ...
    MaybeASTNode Parser::parseVariableDeclaration() {
        ...
        if (optional(VAR_DECL_BEG, true)) {
            ...
            if (optional(ASSIGN)) {
                m_initialization = parseExpression();
                if (!m_initialization.assignTo(initialization))
                    errornext(*this, "Invalid expression in initialization of variable '" + result->getName() + "'.");
                result->setInitialization(initialization);
            } else {
                MaybeASTNode m_declarator = parseDeclarator();
                ASTNode * declarator = nullptr;
                if (!m_declarator.assignTo(declarator))
                    errornext(*this, "Expected type declarator in declaration of variable '" + result->getName() + "'.");
                result->setTypeDeclarator(declarator);
        ...
        if (optional(KWD_PROC, true)) {
            ...
            if (!m_identifier.assignTo(identifier)) {
                ...
                if (m_badDef.assignTo(badDef))
                    errorl(badDef->getContext(), "Anonymous procedures cannot be templates.");
            ...
            MaybeASTNode m_templateDef = parseTemplateDef();
            ASTNode * templateDef = nullptr;
            m_templateDef.assignTo(templateDef); // don't care about the return value of this
            if (templateDef)
                templateResult->setTemplateDef(templateDef);
            
            expect(L_PAREN, result->getFlag(Procedure::IS_ANONYMOUS) ? "identifier or '(' for an anonymous procedure" : "'('");
                ...
                while (true) {
                    ...
                    if (m_param.assignTo(param)) {
                        if (!((VariableDeclaration*)param)->getTypeDeclarator())
                            errorl(param->getContext(), "Procedure parameters must be explicitly typed.");
                        param->setFlag(VariableDeclaration::IS_PROC_PARAM, true);
                    } else {
                        m_param = parseThis();
                        if (!m_param.assignTo(param))
                            errornext(*this, "Expected procedure parameter declaration.");
                    }
            ...
            if (optional(COLON)) {
                ...
                ASTNode * retDeclarator = nullptr;
                if (!m_retDeclarator.assignTo(retDeclarator))
                    errornext(*this, "Expected return type declarator for procedure declaration.");
                result->setRetDeclarator(retDeclarator);
            } else result->setRetDeclarator(newVoidDeclarator());
            ...
            if (optional(L_CURLY_BRACE, true)) {
                ...
                m_statement = parseStatement();
                if (!m_statement.assignTo(statement))
                    errornext(*this, "Expected statement as body of '" + result->getName() + "'.");
                result->addStatement(statement);
            }
    ...
    MaybeASTNode Parser::parseExternSig() {
        if (optional(KWD_EXTERN, true)) {
            ...
            while (true) {
                ...
                if (optional(ELLIPSIS)) {
                    ...
                    if (result->getFlag(Procedure::IS_VARARG))
                        errornext(*this, "An external procedure may only use variadic arguments ('...') as the last argument in the signature.");
                    VariableDeclaration * param = new VariableDeclaration();
                    param->setName(compilation->frontEnd.makeUID(result->getName() + "_arg"));
                    param->getContext().start(&currentContext);
                    MaybeASTNode m_declarator = parseDeclarator();
                    ASTNode * declarator = nullptr;
                    if (!m_declarator.assignTo(declarator))
                        errornext(*this, "Expected type declarator for external procedure argument type.");
                    param->setTypeDeclarator(declarator);
                    param->getContext().finish(&currentContext, &justCleanedContext);
            ...
            if (optional(COLON)) {
                ...
                ASTNode * retDeclarator = nullptr;
                if (!m_retDeclarator.assignTo(retDeclarator))
                    errornext(*this, "Expected return type declarator for external procedure return type.");
                result->setRetDeclarator(retDeclarator);
            } else result->setRetDeclarator(newVoidDeclarator());
    ...
    MaybeASTNode Parser::parseStatement() {
        ...
        (m_statement = parseExpression())           ||
        (m_statement = parseBreak())                ||
        (m_statement = parseContinue())             ||
        (m_statement = parseReturn())               ||
        (m_statement = parsePrint())                ||
        (m_statement = parseIf())                   ||
        (m_statement = parseFor())                  ||
        (m_statement = parseWhile())                ||
        (m_statement = parseDoWhile())              ||
        (m_statement = parseConstant())             ||
    ...
    MaybeASTNode Parser::parseIf() {
        if (optional(KWD_IF, true)) {
            ...
            if (!m_conditional.assignTo(conditional))
                errornext(*this, "Invalid conditional expression for 'if' statement.");
            ...
            if (optional(L_CURLY_BRACE)) {
                ...
                m_statement = parseStatement();
                if (!m_statement.assignTo(statement))
                    errornext(*this, "Expected statement as body in 'if' statement.");
                result->addStatement(statement);
            }
    ...
    MaybeASTNode Parser::parseElse() {
        if (optional(KWD_ELSE, true)) {
            ...
            if (optional(L_CURLY_BRACE)) {
                ...
                m_statement = parseStatement();
                if (!m_statement.assignTo(statement))
                    errornext(*this, "Expected statement as body in 'else' statement.");
                result->addStatement(statement);
            }
    ...
    MaybeASTNode Parser::parseWhile() {
        if (optional(KWD_WHILE, true)) {
            ...
            ASTNode * conditional = nullptr;
            if (!m_conditional.assignTo(conditional))
                errornext(*this, "Invalid conditional expression for 'while' loop.");
            result->setConditional(conditional);
    ...
    MaybeASTNode Parser::parseReturn() {
        if (optional(KWD_RETURN, true)) {
            ...
            if (buff.viewSize() > 0 && !optional(END_OF_LINE)) {
                ...
                ASTNode * expression = nullptr;
                if (!m_expression.assignTo(expression))
                    errornext(*this, "Invalid expression in return statement.", true, "Note: returning without a value must not have anything following 'return' on the same line");
                result->setExpression(expression);
                result->getContext().finish(&currentContext, &justCleanedContext);
            } else {
                result->getContext().finish(&currentContext, &justCleanedContext);
    ...
    MaybeASTNode Parser::parseSLComment() {
        if (optional(HASH, true, true)) {
            ...
            if (newlineIt == buff.end())
                contents = buff;
            else contents = buff.substr(0, newlineIt - buff.begin());
            eat(contents, true);
    ...
    void Parser::moduleCheck(std::vector<ASTNode*>& AST, ModuleDeclaration * module_declared, ModuleDeclaration * mod_decl) {
        if (module_declared) {
            errorl(mod_decl->getContext(), "This compilation unit has already been declared as a module.", false);
            errorl(module_declared->getContext(), "Previous module declaration here.");
        }
        
        if (!AST.empty() && mod_decl != AST.front())
            errorl(mod_decl->getContext(), "Module declarations must be at the beginning of the file/compilation unit.");
    ...
    AsyncParser::AsyncParser(const char * c_str, bool start) : Parser(c_str, start) {  }
    AsyncParser::AsyncParser(std::string& str, bool start) : Parser(str, start) {  }
    AsyncParser::AsyncParser(std::ifstream& file, const std::string& fname, bool start) : Parser(file, fname, start) {  }
    ...
    ImportParser::ImportParser(const char * c_str, bool start) : Parser(c_str, start), mod_decl(nullptr) {  }
    ImportParser::ImportParser(std::string& str, bool start) : Parser(str, start), mod_decl(nullptr) {  }
    ImportParser::ImportParser(std::ifstream& file, const std::string& fname, bool start) : Parser(file, fname, start), mod_decl(nullptr) {  }
