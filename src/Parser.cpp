...
MaybeString parser_kwd_externvar(StringViewableBuffer & buff) {
    return parse_kwd(buff, "externvar");
}
...
MaybeASTNode Parser::parseTopLevelNode() {
    ...
    (m_node = parseModuleDeclaration()) || 
        (m_node = parseProc()) || (m_node = parseExternSig()) || (m_node = parseExternVar()) ||
...
MaybeASTNode Parser::parseExternVar() {
    Context c;
    c.start(&currentContext);
    if (optional(KWD_EXTERNVAR)) {
        MaybeASTNode result = parseVariableDeclaration();
        VariableDeclaration * var = nullptr;

        if (result.assignTo(var)) {
            var->setFlag(VariableDeclaration::IS_EXTERN, true);
            var->getContext().begin = c.begin;
        }
