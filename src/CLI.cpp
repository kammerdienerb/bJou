...
namespace bjou {
    ...
    std::string linenobuf(int ln, bool mark) {
        ...
        } else sprintf(buf, "%10d%s ", ln, side);
        return buf;
    }
    ...
    void _here(Context& context) {
        ...
        if (file) {
            ...
            if (context.begin.line < 3) {
                ...
                std::cout << linenobuf(context.begin.line - 2) << lines[context.begin.line - 3] << "\n";
                std::cout << linenobuf(context.begin.line - 1) << lines[context.begin.line - 2] << "\n";
            }
            ...
            if (context.begin.line == context.end.line) {
                ...
                std::cout << line.substr(context.begin.character - 1, context.end.character - context.begin.character);
                bjouResetColor();
                ...
                auto _lines = bjou::Range<std::string>(&lines[context.begin.line - 1], &lines[context.end.line - 1]);
                ...
                _lines = bjou::Range<std::string>(_lines.begin() + 1, _lines.end() - 2);
                ...
                bjouResetColor();
                std::cout << last.substr(context.end.character - 1, last.size() - 1);
                std::cout << "\n";
            }
            
            if (lines.size() - context.end.line < 3) {
                for (int i = 0; i < lines.size() - context.end.line; i += 1) {
                    std::cout << linenobuf(context.end.line + i + 1) << lines[context.end.line + i] << "\n";
                }
            } else {
                //std::cout << "            " << lines[context.end.line + 1] << "\n";
                //std::cout << "            " << lines[context.end.line + 2] << "\n";
                std::cout << linenobuf(context.end.line + 1) << lines[context.end.line] << "\n";
                std::cout << linenobuf(context.end.line + 2) << lines[context.end.line + 1] << "\n";
            }
        }
    }
	...
	void error(Context context, std::string message, bool exit, std::vector<std::string> continuations) {
		std::lock_guard<std::mutex> lock(cli_mtx);
		_error(context, message);
	...
	void errorl(Context context, std::string message, bool exit, std::vector<std::string> continuations) {
		std::lock_guard<std::mutex> lock(cli_mtx);
		_error(context, message);
		_here(context);
		for (std::string& c : continuations)
	...
	void errornext(Parser& parser, std::string message, bool exit, std::vector<std::string> continuations) {
		Context e_context = errornextGetContext(parser);
	...
	void warning(Context context, std::string message, std::vector<std::string> continuations) {
		std::lock_guard<std::mutex> lock(cli_mtx);
        _error(context, message);
		for (std::string& c : continuations)
			_more(c);
	...
	void warningl(Context context, std::string message, std::vector<std::string> continuations) {
        std::lock_guard<std::mutex> lock(cli_mtx);
        _warning(context, message);
        _here(context);
		for (std::string& c : continuations)
