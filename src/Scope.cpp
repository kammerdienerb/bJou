...
namespace bjou {
    Scope::Scope(std::string _description, Scope * _parent, bool _nspace) : nspace(_nspace), description(_description), parent(_parent) {  }
    
    std::string Scope::mangledPrefix() const {
        return "";
    }
    ...
    NamespaceScope::NamespaceScope(std::string _name, Scope * _parent) : Scope("namespace " + _name, _parent, true), name(_name) {  }
    ...
    static void checkUninitialized(Scope * startingScope, Symbol * sym, Context& context) {
        if (sym->isVar()) {
            if (sym->referenced) {
                Scope * s = startingScope;
                while (s) {
                    if (sym->initializedInScopes.find(s) != sym->initializedInScopes.end())
                        return;
                    s = s->parent;
                }
                errorl(context, "'" + sym->demangledString() + "' might be uninitialized when referenced here.");
            }
        }
    }
    
    Maybe<Symbol*> Scope::getSymbol(Scope * startingScope, ASTNode * _identifier, Context * context, bool traverse, bool fail, bool checkUninit) {
        ...
        if (identifier->namespaces.empty()) {
            ...
            if (traverse && parent)
                return parent->getSymbol(startingScope, _identifier, context, traverse, fail, checkUninit);
        }
        ...
        if (fail) {
            ...
            if (context)
                errorl(*context, "Use of undeclared identifier '" + reportName + "'.");
            else internalError("getSymbol('" + reportName + "') failed without a context.");
        }
        
        return Maybe<Symbol*>();
    }
    
    Maybe<Symbol*> Scope::getSymbol(Scope * startingScope, std::string& qualifiedIdentifier, Context * context, bool traverse, bool fail, bool checkUninit) {
        if (symbols.count(qualifiedIdentifier) > 0) {
            if (checkUninit)
                checkUninitialized(startingScope, symbols[qualifiedIdentifier], *context);
            symbols[qualifiedIdentifier]->referenced = true;
            return Maybe<Symbol*>(symbols[qualifiedIdentifier]);
        }
        if (traverse && parent)
            return parent->getSymbol(startingScope, qualifiedIdentifier, context, traverse, fail, checkUninit);
        if (fail) {
            if (context)
                errorl(*context, "Use of undeclared identifier '" + qualifiedIdentifier + "'.");
            else internalError("getSymbol('" + qualifiedIdentifier + "') failed without a context.");
        }
    ...
    void Scope::addSymbol(Symbol * symbol, Context * context) {
        if ((!parent || nspace) && symbol->name[symbol->name.size() - 1] == '\'')
            errorl(symbol->node()->getNameContext(), "Global symbols using the prime notation are disallowed to maintain cross-language compatibility.", true, "fix: remove trailing '");
            
        Symbol * mangled = symbol->mangled(this);
        Maybe<Symbol*> m_existing;
        if (nspace)
            m_existing = getSymbol(this, symbol->name, nullptr, false, false);
        else m_existing = getSymbol(this, symbol->name, nullptr, true, false);
        Symbol * existing = nullptr;
        if (m_existing.assignTo(existing) && !symbol->node()->getFlag(ASTNode::SYMBOL_OVERWRITE)) {
            if (existing->node()->nodeKind == ASTNode::PROC_SET) {
                ...
                for (auto& psym : procSet->procs) {
                    if (psym.second->node()->getScope() == this) {
                        errorl(*context, "Redefinition of '" + mangled->demangledString() + "' as a different kind of symbol.", false);
                        Context econtext = psym.second->node()->getNameContext();
                        errorl(econtext, "'" + psym.second->demangledString() + "' defined here.");
                    }
                }
            } else {
                errorl(*context, "Redefinition of '" + mangled->demangledString() + "'.", false);
                Context econtext = existing->node()->getNameContext();
                errorl(econtext, "'" + existing->demangledString() + "' also defined here.");
            }
        }
        if (symbol->isVar() && symbol->node()->getFlag(VariableDeclaration::IS_PROC_PARAM))
    ...
    void Scope::addSymbol(_Symbol<Procedure> * symbol, Context * context) {
        ...
        Symbol * existing = nullptr;
        if (m_existing.assignTo(existing) && existing->node()->nodeKind != ASTNode::PROC_SET) {
            if (existing->node()->getFlag(ASTNode::SYMBOL_OVERWRITE)) {
                existing->node()->setFlag(ASTNode::IGNORE_GEN, true);
            } else {
                errorl(*context, "Redefinition of '" + mangled->demangledString() + "' as a different kind of symbol.", false);
                errorl(existing->node()->getNameContext(), "'" + existing->demangledString() + "' defined here.");
            }
        }
        
        
        Maybe<Symbol*> m_mangled_existing = getSymbol(this, mangled->name, nullptr, false, false);
        ...
        if (m_mangled_existing.assignTo(mangled_existing)) {
            ...
            } else if (mangled_existing->node()->nodeKind == ASTNode::PROC_SET && mangled->node()->getFlag(Procedure::IS_EXTERN)) {
                // @incomplete
                // Create a mechanism to catch conflicting redefinitions of extern procs
            } else {
                errorl(*context, "Redefinition of '" + mangled->demangledString() + "'.", false);
                errorl(mangled_existing->node()->getNameContext(), "'" + mangled_existing->demangledString() + "' also defined here.");
            }
        }
        ...
        if (!parent || (nspace && !parent->parent)) {
            // global ProcSets are only concerned with one layer of namespace deduction
            if (compilation->frontEnd.globalScope->symbols.count(name) == 0)
                compilation->frontEnd.globalScope->symbols[name] = new _Symbol<ProcSet>(name, new ProcSet(name));
            
            if (compilation->frontEnd.globalScope->symbols[name]->node()->nodeKind == ASTNode::PROC_SET) {
                ProcSet * set = (ProcSet*)compilation->frontEnd.globalScope->symbols[name]->node();
            ...
            if (proc->getFlag(Procedure::IS_INTERFACE_IMPL)) {
                ...
                Identifier * ident = (Identifier*)((InterfaceImplementation*)proc->parent)->getIdentifier();
                iname += mangledIdentifier(ident) + "." + proc->getName();
                
                if (compilation->frontEnd.globalScope->symbols.count(iname) == 0)
                    compilation->frontEnd.globalScope->symbols[iname] = new _Symbol<ProcSet>(iname, new ProcSet(iname));
                
                if (compilation->frontEnd.globalScope->symbols[iname]->node()->nodeKind == ASTNode::PROC_SET) {
                    ProcSet * set = (ProcSet*)compilation->frontEnd.globalScope->symbols[iname]->node();
                    set->procs[mangled->name] = mangled;
                }
            }
        }
        ...
        if (nspace) {
            ...
            if (proc->getFlag(Procedure::IS_INTERFACE_IMPL)) {
                ...
                if (symbols.count(name) == 0)
                    symbols[iname] = new _Symbol<ProcSet>(iname, new ProcSet(iname));
        ...
        // if it's extern and the mangled name is the same, we don't want to clobber the set
    ...
    void createProcSetsForPuntedInterfaceImpl(Struct * s, InterfaceImplementation * impl) {
    ...
    void createProcSetForInheritedInterfaceImpl(Struct * s, InterfaceImplementation * impl) {
        Identifier * ident = (Identifier*)impl->getIdentifier();
        Maybe<Symbol*> m_sym = s->getScope()->getSymbol(s->getScope(), ident);
        Symbol * sym = nullptr;
        m_sym.assignTo(sym);
        ...
        BJOU_DEBUG_ASSERT(impl->parent && impl->parent->nodeKind == ASTNode::STRUCT);
        ...
        for (auto& procs : idef->getProcs()) {
            ...
            compilation->frontEnd.globalScope->symbols[name] = new _Symbol<ProcSet>(name, set);
            
            std::string lookup = implementor_struct->getMangledName() + "." + procs.first;
            Maybe<Symbol*> m_sym = implementor_struct->getScope()->getSymbol(implementor_struct->getScope(), lookup);
            Symbol * sym = nullptr;
            m_sym.assignTo(sym);
            BJOU_DEBUG_ASSERT(sym);
    ...
    void Scope::printSymbols(int indent) const {
        ...
        printf("%s %-76s %s\n", side, (std::string(indent * 2, ' ') + "*** " + description + ":").c_str(), side);
