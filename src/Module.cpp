...
namespace bjou {
    static void pushImportsFromAST(std::vector<ASTNode*>& AST, std::deque<Import*>& imports) {
    ...
    static void importModules(std::deque<Import*> imports, std::vector<ASTNode*>& AST) {
        std::set<std::string>& modulesImported = compilation->frontEnd.modulesImported;
        ...
        if (nthreaded > 0 && !compilation->args.noparallel_arg.getValue()) {
            ...
            while (!imports.empty()) {
                while (!imports.empty() && (imports.size() < nthreaded || parserContainer.size() < nthreaded)) {
                    ...
                    if (filesSeen.find(fname) == filesSeen.end()) {
                        ...
                        if (!parser->mod_decl)
                            error(import->getContext(), "File '" + fname + "' does not declare a module.");
                        ...
                        if (modulesImported.find(parser->mod_decl->getIdentifier()) == modulesImported.end()) {
                            ...
                            futureTimes[fname] = std::async(std::launch::async, std::ref(*parserContainer.back()));
                        } else {
                            parser->Dispose();
                            delete parser;
                        }
            ...
            while (!imports.empty()) {
                ...
                if (filesSeen.find(fname) == filesSeen.end()) {
                    ...
                    if (modulesImported.find(parser.mod_decl->getIdentifier()) == modulesImported.end()) {
                        ...
                        AST.insert(AST.end(), parser.nodes.begin(), parser.nodes.end());
                    }
                    else parser.Dispose();
                }
            }
    ...
    AsyncImporter::AsyncImporter(std::string& _header, std::ifstream * _ifs, boost::archive::text_iarchive * _ar) : header(_header), ifs(_ifs), ar(_ar) {  }
    ...
    void importModules(std::vector<ASTNode*>& AST) {
        ...
        if (nimports > 1 && nthreaded > 0 && !compilation->args.noparallel_arg.getValue()) {
            unsigned int i = 0;
            while (i < nimports) {
                // do threads
                ...
                while (importers.size() < nthreaded || (i >= (nimports - (nimports % nthreaded)) && i < nimports)) {
                    ...
                    // de-quote
                    std::string fname = import->module.substr(1, import->module.size() - 2);
                    
                    std::ifstream * ifs = new std::ifstream(fname);
                    BJOU_DEBUG_ASSERT(ifs && "Bad file!");
                    boost::archive::text_iarchive * ar = new boost::archive::text_iarchive(*ifs);
                    ...
                    if (modulesImported.find(header) == modulesImported.end()) {
                        ...
                        futureTimes[header] = std::async(std::launch::async, std::ref(importers.back()));
                        ...
                        for (AsyncImporter& importer : importers) {
                            ...
                            AST.insert(AST.end(), importer.AST.begin(), importer.AST.end());
    ...
    void exportModule(std::vector<ASTNode*>& AST) {
        exportModuleToFile(AST, (compilation->outputpath + compilation->outputbasefilename).c_str());
    }
    ...
    void joinTypeTableWithCompilation(std::unordered_map<std::string, Type*>& typeTable) {
        // @leak?
    ...
    void joinImportedIDsWithCompilation(std::set<std::string>& modulesImported) {
        compilation->frontEnd.modulesImported.insert(modulesImported.begin(), modulesImported.end());
    }
     */
}
