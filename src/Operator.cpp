...
int precedence(const char * op) { return getOpInfo(op, OpMap)->precedence; }

bool unary(const char * op) { return getOpInfo(op, OpMap)->unary; }

bool binary(const char * op) { return !getOpInfo(op, OpMap)->unary; }
...
bool isAssignableOp(std::string & op) {
    return s_in_a(op.c_str(), assignableOps);
}
