...
namespace bjou {
    ...
    Type::Type(Kind _kind, std::string _name, Sign _sign, int _size) : kind(_kind), sign(_sign), size(_size), code(_name) {  }

	bool Type::isValid() const 			{ return kind != INVALID;   		}
    bool Type::isPrimative() const  	{ return kind == PRIMATIVE; 		}
    bool Type::isFP() const             { return    code == "float"     ||
                                                    code == "double"    ||
                                                    code == "f32"       ||
                                                    code == "f64";          }
    ...
    bool Type::enumerableEquivalent() const {
        const char * enumerables[] = {
            "bool", "u8", "i8", "u16", "i16",
            "u32", "i32", "u64", "i64",
            "int", "uint", "long", "ulong", "char", "short"
        };
    ...
    bool Type::equivalent(const Type * other, bool exactMatch) const {
        if (isPrimative() && other->isPrimative()) {
            ...
            else return (getOriginal()->code == other->getOriginal()->code) || (enumerableEquivalent() && other->enumerableEquivalent());
        }
        return false;
    }
    ...
    bool Type::isValidOperand(const Type * operand, std::string& op) const {
        ...
        if (s_in_a(op.c_str(), same))
            return equivalent(operand) || (enumerableEquivalent() && operand->enumerableEquivalent());
    ...
    const Type * Type::replacePlaceholders(const Type * t) const { return new Type(*this); }
    ...
    PlaceholderType::PlaceholderType() : Type(PLACEHOLDER) {
        code = "_";
    }
    
    const Type * PlaceholderType::replacePlaceholders(const Type * t) const { return t; }
    ...
    StructType::StructType(std::string& name, Struct * __struct, TemplateInstantiation * _inst) : Type(STRUCT, name), isAbstract(__struct->getFlag(Struct::IS_ABSTRACT)), _struct(__struct), inst(_inst), extends(nullptr) {  }
    
    static void insertProcSet(StructType * This, Procedure * proc) {
        // if (This->memberProcs.find(proc->getName()) == This->memberProcs.end()) {
            std::string lookup = This->_struct->getMangledName() + "." + proc->getName();
            Maybe<Symbol*> m_sym = This->_struct->getScope()->getSymbol(This->_struct->getScope(), lookup);
            Symbol * sym = nullptr;
            m_sym.assignTo(sym);
    ...
    const Type * StructType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = { "=" };
        const char * boolean[] = { "==", "!=" };
    ...
    const Type * StructType::replacePlaceholders(const Type * t) const { return new StructType(*this); }
    ...
    AliasType::AliasType(std::string& name, Alias * _alias) : Type(ALIAS, name), alias(_alias) {
        code = name;
    }
    ...
    const Type * AliasType::binResultType(const Type * operand, std::string& op) const {
    ...
    void AliasType::complete() {
        alias_of = alias->getDeclarator()->getType();
    }
    
    
    ArrayType::ArrayType() : Type(ARRAY) {  }
    
    ArrayType::ArrayType(const Type * _array_of) : Type(ARRAY), array_of(_array_of), expression(nullptr), size(-1) {
        code = array_of->code + "[]";
    }
    
    ArrayType::ArrayType(const Type * _array_of, Expression * _expression) : Type(ARRAY), array_of(_array_of), expression(_expression) {
        code = array_of->code + "[]";
    ...
    bool ArrayType::equivalent(const Type * other, bool exactMatch) const {
        ...
        ( (other->isPointer() && array_of->equivalent(((PointerType*)other)->pointer_of, exactMatch)));
    }
    
    const Type * ArrayType::getBase() const {
        return array_of->getBase();
    }
    
    Declarator * ArrayType::getGenericDeclarator() const {
        ArrayDeclarator * decl = new ArrayDeclarator(array_of->getGenericDeclarator(), expression);
        decl->size = size;
    ...
    const Type * ArrayType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "+", "-", "="
        };
    ...
    DynamicArrayType::DynamicArrayType(const Type * _array_of) : Type(DYNAMIC_ARRAY), array_of(_array_of) {
        code = array_of->code + "[...]";
    }
    ...
    const Type * DynamicArrayType::getBase() const {
        return array_of->getBase();
    }
    
    Declarator * DynamicArrayType::getGenericDeclarator() const {
        DynamicArrayDeclarator * decl = new DynamicArrayDeclarator(array_of->getGenericDeclarator());
        decl->createdFromType = true;
        return decl;
    }
    ...
    bool DynamicArrayType::isValidOperand(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "+", "-", "==", "!=", "="
        };
    ...
    const Type * DynamicArrayType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "+", "-", "="
        };
    ...
    PointerType::PointerType(const Type * _pointer_of) : Type(POINTER), pointer_of(_pointer_of) {
        code = pointer_of->code + "*";
    }
    
    bool PointerType::equivalent(const Type * other, bool exactMatch) const {
        if (other->isPointer())
            if (pointer_of->equivalent(((PointerType*)other)->pointer_of, exactMatch))
                return true;
        if (other->isArray() && ((ArrayType*)other)->array_of->equivalent(pointer_of, exactMatch))
            return true;
    ...
    const Type * PointerType::getBase() const {
        return pointer_of->getBase();
    }
    
    Declarator * PointerType::getGenericDeclarator() const {
        PointerDeclarator * decl = new PointerDeclarator(pointer_of->getGenericDeclarator());
        decl->createdFromType = true;
        return decl;
    }
    ...
    const Type * PointerType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            // "+", "-",
            "="
    ...
    MaybeType::MaybeType(const Type * _maybe_of) : Type(MAYBE), maybe_of(_maybe_of) {
        code = maybe_of->code + "?";
    }
    
    const Type * MaybeType::getBase() const {
        return maybe_of->getBase();
    }
    
    Declarator * MaybeType::getGenericDeclarator() const {
        MaybeDeclarator * decl = new MaybeDeclarator(maybe_of->getGenericDeclarator());
        decl->createdFromType = true;
        return decl;
    }
    ...
    const Type * MaybeType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "="
        };
    ...
    TupleType::TupleType(std::vector<const Type*> _subTypes) : Type(TUPLE), subTypes(_subTypes) {
        code = "(";
    ...
    const Type * TupleType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "="
        };
    ...
    ProcedureType::ProcedureType(std::vector<const Type*> _paramTypes, const Type * _retType, bool _isVararg) :
        Type(PROCEDURE), paramTypes(_paramTypes), isVararg(_isVararg), retType(_retType) {
        code = "<(";
    ...
    bool ProcedureType::argMatch(const Type * _other, bool exactMatch) {
        ...
        size_t min_nparams = my_nparams <= other_nparams ? my_nparams : other_nparams;
    ...
    std::string ProcedureType::getDemangledName() const {
        ...
        if (isVararg) demangled += "...";
        demangled += ")";
        if (retType->getDemangledName() != compilation->frontEnd.getBuiltinVoidTypeName())
    ...
    bool ProcedureType::isValidOperand(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        BJOU_DEBUG_ASSERT(op != "()" && "Operand for call must be ArgList, which does not have a type.");
        const char * same[] = {
            "==", "!=", "="
        };
    ...
    const Type * ProcedureType::binResultType(const Type * operand, std::string& op) const {
        BJOU_DEBUG_ASSERT(opApplies(op) && binary(op.c_str()));
        const char * same[] = {
            "="
        };
    ...
    TemplateStructType::TemplateStructType(std::string& name) : Type(TEMPLATE_STRUCT, name) {
    ...
    TemplateAliasType::TemplateAliasType(std::string& name) : Type(TEMPLATE_ALIAS, name) {
    ...
    void compilationAddPrimativeTypes() {
        ...
        for (auto& p : primatives)
            compilation->frontEnd.primativeTypeTable[std::get<0>(p)] = new Type(Type::PRIMATIVE, std::get<0>(p), std::get<1>(p), std::get<2>(p));
    }
    ...
    std::vector<const Type*> typesSortedByDepencencies(std::vector<const Type*> nonPrimatives) {
        ...
        while (sorted.size() < size) {
            ...
            for (const Type * t : nonPrimatives) {
                ...
                // break it down to the declarators and expressions that would require knowledge of
                // a certain type beyond its name
                if (t->isStruct()) {
                    ...
                    for (ASTNode * _mem : td->getMemberVarDecls()) {
                        ...
                        // declarators with a pointer ANYWHERE in them imply that we don't
                        // need to know the size of the base type
                        // PointerDeclarator handles this on construction and sets a flag in
                        // their respective base Declarator
                        ...
                        if (mem->getInitialization()) {
                            ...
                            for (ASTNode * term : init_sub_nodes)
                                if (term->nodeKind >= ASTNode::DECLARATOR && term->nodeKind < ASTNode::_END_DECLARATORS)
                                    if (((Declarator*)term)->getBase()->getFlag(Declarator::IMPLIES_COMPLETE))
                ...
                for (ASTNode * term : terms) {
                    ...
                    if (t->getBase()->isStruct() || t->getBase()->isAlias()) {
                        // has the type that this term needs already been placed in 'sorted'?
                        if (std::find(availableByCode.begin(), availableByCode.end(), t->getBase()->code) == availableByCode.end()) {
                            good = false;
                            if (!badTerm)
            ...
            for (unsigned int i = 0; i < nNonPrimatives; i += 1) {
                ...
                } else keep.push_back(nonPrimatives[i]);
            }
            ...
            // The problem isn't that a type wasn't "complete" -- the whole point of this
            // algorithm is to make incomplete types impossible. However, we can't solve
            // circular dependencies.
            if (nadded == 0) {
                Context t_context, ref_context;
                if (nonPrimatives[0]->isStruct()) {
                    t_context = ((StructType*)nonPrimatives[0])->_struct->getNameContext();
                } else if (nonPrimatives[0]->isAlias()) {
                    // @todo add suport for aliases
                }
                std::string demangledA = demangledString(nonPrimatives[0]->code);
                std::string demangledB = demangledString(badTerm->getType()->getBase()->code); // @leak
                ref_context = badTerm->getContext();
                errorl(t_context, "Definition of type '" + demangledA + "' creates a circular dependency with type '" + demangledB + "'.", false);
                if (extends_err)
                    errorl(ref_context, "'" + demangledB + "' referenced here.", true, "Note: types may not extend themselves.");
                else errorl(ref_context, "'" + demangledB + "' referenced here.", true, "Did you mean to reference '" + demangledB + "' by pointer?");
            }
        }
