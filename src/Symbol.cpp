    ...
    */
    ...
    Procedure * ProcSet::get(ASTNode * args, ASTNode * inst, Context * context, bool fail) {
		...
		if (fail && !compare_type) {
			std::string help1 = "Note: overloads are selected based on current l-value type.";
			std::string help2 = "\te.g. 'var : <(int, int) : int> = sum' will be able to select the correct overload if it exists";
			errorl(*context, "Reference to '" + name + "' is ambiguous.", true, help1, help2);
		}	 
    ...
    Procedure * ProcSet::getTemplate(std::vector<const Type*>& arg_types, ASTNode * args, ASTNode * inst, Context * context, bool fail) {
        ...
        for (auto& p : procs) {
            if (p.second->isTemplateProc()) {
                ...
                if (checkTemplateProcInstantiation(tproc, args, inst, context, fail) != -1) {
	...
	std::vector<Symbol*> ProcSet::getCandidates(ProcedureType * compare_type, ASTNode * args, ASTNode * inst, Context * context, bool fail) {
	...
	static int countConversions(ProcedureType * compare_type, ProcedureType * candidate_type) {
		int nconv = 0;
		for (int i = 0; i < (int)candidate_type->paramTypes.size(); i += 1) {
			BJOU_DEBUG_ASSERT(compare_type->paramTypes[i]->equivalent(candidate_type->paramTypes[i]));
			if (!compare_type->paramTypes[i]->equivalent(candidate_type->paramTypes[i], /* exact_match =*/true))
				nconv += 1;
		}
		return nconv;
	}

	bool ProcSet::resolve(std::vector<Symbol*>& candidates, std::vector<Symbol*>& resolved, ProcedureType * compare_type, ASTNode * args, ASTNode * inst, Context * context, bool fail) {
	...
	void ProcSet::showCandidatesError(std::vector<Symbol*>& candidates, Context * context) {
		errorl(*context, "Reference to '" + name + "' is ambiguous.", false);
		for (Symbol * &sym : candidates) {
			std::string show;
			
			if (sym->isTemplateProc())
				show = ((Procedure*)((TemplateProc*)sym->node())->_template)->getMangledName();
			else show = ((Procedure*)sym->node())->getMangledName();

			show = demangledString(show);

			errorl(sym->node()->getNameContext(), "'" + show + "' is a candidate.", (&sym == &candidates.back()));
		}
	}
