...
namespace bjou {
    Compilation::Compilation(FrontEnd& _frontEnd, BackEnd& _backEnd, TCLAPArgSet& _args)
        : max_interface_procs(0), frontEnd(_frontEnd), backEnd(_backEnd), args(_args) {
            ...
            if (args.output_arg.getValue().size()) {
                ...
                outputpath = o.find('/') == std::string::npos ? "" : o.substr(0, o.find_last_of('/')) + "/";
                outputbasefilename = o.substr(0, o.find_last_of('.'));
                outputbasefilename = outputbasefilename.substr(outputbasefilename.find_last_of("/") + 1);
            } else {
