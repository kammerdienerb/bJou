...
namespace bjou {
    static void fix_typeinfo_v_table_size(FrontEnd& frontEnd) {
        ...
        // Fix type
        Type * _v_table_t = (Type*)s_t->memberTypes[s_t->memberIndices["_v_table"]];
        ...
        BJOU_DEBUG_ASSERT(v_table_t->expression->nodeKind == ASTNode::INTEGER_LITERAL);
        v_table_t->expression = (Expression*)v_table_t->expression->clone();
        v_table_t->expression->setContents(std::to_string(compilation->max_interface_procs));
    ...
    milliseconds FrontEnd::go() {
        ...
        if (time_arg) {
            ...
            prettyPrintTimeMin(p_time, "Parsed " + std::to_string(nfiles) + (nfiles == 1 ? " file" : " files"));
        }
    ...
    milliseconds FrontEnd::ParseStage() {
        ...
        const std::vector<std::string>& fnames = compilation->args.files.getValue();
        size_t nfiles = fnames.size();
        
        std::unordered_map<std::string, milliseconds> times;
        
        if (nfiles > 1  && nthreaded > 0 && !compilation->args.noparallel_arg.getValue()) {
            // do threads
            ...
            for (unsigned long i = nfiles - (nfiles % nthreaded); i < nfiles; i += 1) {
