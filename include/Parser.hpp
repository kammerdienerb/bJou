...
namespace bjou {
    ...
    enum TokenKind {
        GENERIC_TOKEN, NON_COMMENT, IDENTIFIER, IDENTIFIER_ALLOW_PRIMATIVE_TYPES,
        ANY_CHAR, SEMICOLON, UNDER_SCORE, COMMA, ELLIPSIS,
        COLON, DBL_COLON, BACK_SLASH, DOT_COLON, VERT,
        HASH, THICK_ARROW, DOLLAR, QUESTION,
        DBL_QUESTION, TEMPLATE_BEGIN, KWD_CONST, KWD_TYPE, KWD_ABSTRACT,
        KWD_EXTENDS, KWD_INTERFACE, KWD_IMPLEMENTS, KWD_FROM,
        KWD_ENUM, KWD_PRINT, KWD_RAW, KWD_IMMUT,
        KWD_COERCE, KWD_THIS, KWD_DELETE, KWD_NAMESPACE, KWD_IMPORT, KWD_MODULE,
        KWD_ALIAS, KWD_OPERATOR, KWD_RETURN, KWD_IF,
        KWD_ELSE, KWD_WHILE, KWD_DO, KWD_FOR,
        KWD_MATCH, KWD_WITH, KWD_BREAK, KWD_CONTINUE,
        L_CURLY_BRACE, R_CURLY_BRACE, L_SQR_BRACKET, R_SQR_BRACKET,
        L_PAREN, R_PAREN, INTEGER, FLOATING_PT,
        CHAR_LITERAL, STRING_LITERAL, KWD_TRUE, KWD_FALSE,
        KWD_NOTHING, KWD_AS, DOT, ARROW,
        EXCLAM, KWD_SIZEOF, AMP, AT,
        KWD_NOT, KWD_NEW, KWD_PROC, KWD_EXTERN,
        KWD_SOME, MULT, DIV, MOD,
        PLUS, MINUS, LSS, LEQ,
        GTR, GEQ, EQU, NEQ,
        AND, KWD_AND, OR, KWD_OR,
        ASSIGN, PLUS_EQ, MIN_EQ, MULT_EQ,
        DIV_EQ, MOD_EQ, VAR_DECL_BEG, // CONSTANT_DECL_BEG, // @const
        SL_COMMENT_BEG, ML_COMMENT_BEG, ML_COMMENT_END, END_OF_LINE
    };
    ...
    struct Parser {
        ...
        MaybeString text(std::string txt, bool skipEat = false, bool skipClean = false);
        MaybeString optional(TokenKind tok, bool skipEat = false, bool skipClean = false);
        std::string expect(TokenKind tok, std::string err, bool skipEat = false, bool skipClean = false);
        
        template <typename... continuations>
        std::string expect(TokenKind tok, std::string err, bool skipEat, bool skipClean, continuations... more) {
        ...
        MaybeASTNode parseQualifiedIdentifier(bool allow_primative_typenames = false);
        ...
        ASTNode * applyPostfixOp(std::string& postfixOp, Expression * expr, Expression * left, Expression * right, MaybeString& m_binOp, int minPrecedence);
        ...
        void moduleCheck(std::vector<ASTNode*>& AST, ModuleDeclaration * module_declared, ModuleDeclaration * mod_decl);
    };
    
    struct AsyncParser : Parser {
        AsyncParser(const char * c_str, bool start = true);
        AsyncParser(std::string& str, bool start = true);
        AsyncParser(std::ifstream& file, const std::string& fname, bool start = true);
    ...
    struct ImportParser : Parser {
        ...
        ImportParser(std::ifstream& file, const std::string& fname, bool start = true);
