...
template <typename T> struct _Symbol : Symbol {
    _Symbol(std::string _name, ASTNode * __node, ASTNode * _inst = nullptr)
        : Symbol(_name, __node, _inst) {}
...
template <> inline std::string _Symbol<Struct>::mangledString(Scope * scope) {
    std::string _name = name;
    ...
    if (!proc->getFlag(Procedure::IS_EXTERN)) {
        ...
        if (proc->getFlag(Procedure::IS_TYPE_MEMBER)) {
            ...
            if (proc->getFlag(Procedure::IS_INTERFACE_IMPL)) {
                BJOU_DEBUG_ASSERT(proc->parent->nodeKind ==
                                  ASTNode::INTERFACE_IMPLEMENTATION);
                InterfaceImplementation * impl =
                    (InterfaceImplementation *)proc->parent;
                _name += mangledInterfaceImplPrefix(impl);
            }
        }
        std::string m_inst;
        if (inst)
            m_inst = mangledInst(inst);
        _name +=
            std::to_string(name.size()) + name + m_inst + mangledParams(proc);
    }

    return _name;
}
template <> inline Symbol * _Symbol<Procedure>::mangled(Scope * scope) {
    _Symbol<Procedure> * mangledSymbol =
        new _Symbol<Procedure>(name, _node, inst);
    mangledSymbol->name = mangledString(scope);
    return mangledSymbol;
}
...
inline std::string _Symbol<TemplateProc>::mangledString(Scope * scope) {
    ...
    TemplateDefineList * def =
        (TemplateDefineList *)template_proc->getTemplateDef();
    ...
    _name += std::to_string(name.size()) + name + "M" +
             std::to_string(def->getElements().size()) +
...
template <> inline Symbol * _Symbol<TemplateProc>::mangled(Scope * scope) {
    _Symbol<TemplateProc> * mangledSymbol =
        new _Symbol<TemplateProc>(name, _node);
    mangledSymbol->name = mangledString(scope);
    return mangledSymbol;
}

template <> inline std::string _Symbol<ProcSet>::mangledString(Scope * scope) {
    std::string _name = name;
    std::string prefix =
        "_Z" + scope->mangledPrefix() + std::to_string(_name.size());
    if (prefix.size() > std::strlen("_Z"))
        _name = prefix + _name;
    return _name;
}
template <> inline Symbol * _Symbol<ProcSet>::mangled(Scope * scope) {
    _Symbol<ProcSet> * mangledSymbol = new _Symbol<ProcSet>(name, _node);
    mangledSymbol->mangledString(scope);
    return mangledSymbol;
}

template <> inline std::string _Symbol<Constant>::mangledString(Scope * scope) {
    std::string _name = name;
    std::string prefix = scope->mangledPrefix();
...
template <> inline Symbol * _Symbol<Constant>::mangled(Scope * scope) {
...
inline Symbol * _Symbol<VariableDeclaration>::mangled(Scope * scope) {
    _Symbol<VariableDeclaration> * mangledSymbol =
        new _Symbol<VariableDeclaration>(name, _node);
    mangledSymbol->name = mangledString(scope);
    return mangledSymbol;
}
} // namespace bjou
