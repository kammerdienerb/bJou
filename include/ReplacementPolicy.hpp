...
namespace bjou {
    ...
    template <typename T>
    replacementPolicy * rpget();

    #define RP_FUNCTOR_DECL(name) struct replacementPolicy_##name : replacementPolicy {         \
        ASTNode * operator()(ASTNode * parent, ASTNode * old_node, ASTNode * new_node);         \
    };                                                                                          \
