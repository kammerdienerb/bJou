...
namespace bjou {
    ...
    void _error(Context& context, std::string message) {
        bjouSetColor(CYAN);
        std::cout <<
        "bJou :: " << context.filename << " :: " << context.begin.line << " :: " << context.begin.character << "\n";
        bjouSetColor(RED);
        std::cout << "     Error: ";
        bjouSetColor(GREEN);
        std::cout << message;
	...
	void error(Context context, std::string message, std::vector<std::string> continuations);

    template <typename... continuations>
    void error(Context context, std::string message, bool exit, continuations... c) {
    ...
    template <typename... continuations>
    void errorl(Context context, std::string message, bool exit, continuations... c) {
        std::lock_guard<std::mutex> lock(cli_mtx);
        _error(context, message);
        _more(c...);
        _here(context);
    ...
    template <typename... continuations>
    void errornext(Parser& parser, std::string message, bool exit, continuations... c) {
